  ...
  if (req.cookies.user) {
    ...
    axios({
      ...
      params: {
        ...
        storeId: req.body.store ? req.body.store : null
      },
    })
    .then((response) => {
      if (req.body.store != null) {
        ...
        .then(axios.spread((storesRes, manufacturersRes, inventoryRes) => {
          ...
          res.render("accessories/index", {stores: storesRes.data, manufacturers: manufacturersRes.data, data: storeInventory, currStore: req.body.store});
        }))
        .catch(err => {
          console.log(err);
          if (err.response.status == 401) {
